...
import { isParagraphActive } from "@/components/tiptap-ui/paragraph-button"
...
export function useHeadingDropdownMenu(config?: UseHeadingDropdownMenuConfig) {
  ...
  const paragraphActive = isParagraphActive(editor)
  const isActive = isHeadingActive(editor) || paragraphActive
  ...
  return {
    ...
    levels,
    label: paragraphActive ? "Paragraph" : "Heading",
    tooltipLabel: "Text style",
    Icon: paragraphActive
      ? ParagraphIcon
      : activeLevel
